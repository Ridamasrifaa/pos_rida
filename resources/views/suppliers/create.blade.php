...
@section('title', 'Tambah Suplier - POS Rida')
...
<div class="w-full max-w-4xl mx-auto space-y-6 font-sans py-6 px-4 animate-fadeIn">
    <div class="bg-white p-6 rounded-3xl shadow-md border border-slate-200 transition-all duration-300 hover:shadow-lg">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900">Tambah Suplier Baru</h1>
            <p class="text-sm font-normal text-slate-600 mt-0.5">Masukkan informasi distributor atau sales tempat kulakan barang.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-md border border-slate-200 p-8 transition-all duration-300 hover:shadow-lg">
        <form action="{{ route('suppliers.store') }}" method="POST" class="space-y-5">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="phone" id="phone" value="{{ old('phone') }}" placeholder="Contoh: 081234567890" class="w-full px-4 py-3 bg-white border border-slate-300 rounded-2xl text-sm font-medium text-slate-800 placeholder-slate-400 focus:outline-none focus:border-rose-600 focus:ring-2 focus:ring-rose-600/20 transition-all duration-200 shadow-sm">
                @error('phone')
                    <span class="text-xs text-rose-600 mt-1 block font-medium">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea name="address" id="address" rows="3" placeholder="Contoh: Jl. Perintis Kemerdekaan No. 10, Tasikmalaya" class="w-full px-4 py-3 bg-white border border-slate-300 rounded-2xl text-sm font-medium text-slate-800 placeholder-slate-400 focus:outline-none focus:border-rose-600 focus:ring-2 focus:ring-rose-600/20 transition-all duration-200 shadow-sm">{{ old('address') }}</textarea>
                @error('address')
                    <span class="text-xs text-rose-600 mt-1 block font-medium">{{ $message }}</span>
                @enderror
            </div>
