...
<head>
    ...
    <style>
        ...
        @keyframes clickJump {
            ...
            30% { transform: translateY(-20px) scale(1.15) rotate(-8deg); filter: drop-shadow(0 15px 15px rgba(225, 29, 72, 0.3)); }
            60% { transform: translateY(5px) scale(0.95) rotate(6deg); }
...
<body class="bg-gradient-to-br from-rose-50 via-slate-50 to-rose-100/40 h-screen flex items-center justify-center p-4 font-sans selection:bg-rose-600 selection:text-white">
    ...
    <div class="w-full max-w-md animate-card relative z-10 flex flex-col items-center">
        ...
        <div id="cartoonCharacter" class="floating-cartoon mb-[-30px] z-20 pointer-events-none drop-shadow-xl">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 120" class="w-28 h-28">
                ...
                <path d="M57 80 Q60 84 63 80" stroke="#1e293b" stroke-width="2.5" stroke-linecap="round" fill="none"/>
                ...
                <line x1="60" y1="28" x2="60" y2="14" stroke="#e11d48" stroke-width="3" stroke-linecap="round"/>
                <circle cx="60" cy="12" r="5" fill="#fda4af"/>
            </svg>
        </div>

        <!-- Card Utama -->
        <div class="w-full bg-white/95 backdrop-blur-xl shadow-2xl shadow-rose-950/10 border border-white rounded-[2.5rem] overflow-hidden">
                ...
                <form id="loginForm" action="{{ route('auth') }}" method="POST" class="space-y-4">
                    ...
                    <div class="form-control">
                        <label class="label pb-1.5">
                            <span class="label-text text-[11px] font-bold uppercase tracking-wider text-slate-600">Email</span>
                        </label>
                        <label class="input input-bordered rounded-2xl flex items-center gap-3 bg-slate-50/60 focus-within:bg-white focus-within:border-rose-600 focus-within:ring-4 focus-within:ring-rose-600/10 transition-all h-12">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-4 h-4 text-slate-400"><path d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6ZM12.735 14c.618 0 1.093-.561.872-1.139a6.002 6.002 0 00-11.215 0c-.22.578.254 1.139.872 1.139h9.47Z" /></svg>
                            <input type="email" name="email" value="{{ old('email') }}" required placeholder="nama@email.com" class="grow text-sm bg-transparent focus:outline-none text-slate-800" />
                        </label>
                        @error('email')
                    ...
                    <div class="form-control">
                        <label class="label pb-1.5">
                            <span class="label-text text-[11px] font-bold uppercase tracking-wider text-slate-600">Kata Sandi</span>
  
                        </label>
                        <label class="input input-bordered rounded-2xl flex items-center gap-3 bg-slate-50/60 focus-within:bg-white focus-within:border-rose-600 focus-within:ring-4 focus-within:ring-rose-600/10 transition-all h-12">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-4 h-4 text-slate-400"><path fill-rule="evenodd" d="M14 6a4 4 0 0 1-4.899 3.899l-1.955 1.955a.5.5 0 0 1-.353.146H5v1.5a.5.5 0 0 1-.5.5h-2a.5.5 0 0 1-.5-.5v-2.293a.5.5 0 0 1 .146-.353l3.955-3.955A4 4 0 1 1 14 6Zm-4-2a.75.75 0 0 0 0 1.5.5.5 0 0 1 .5.5.75.75 0 0 0 1.5 0 2 2 0 0 0-2-2Z" clip-rule="evenodd" /></svg>
                            <input type="password" name="password" required placeholder="••••••••" class="grow text-sm bg-transparent focus:outline-none text-slate-800" />
                        </label>
                        @error('password')
                            <span class="text-rose-600 text-xs font-medium mt-1">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-control pt-1">
                        <label class="label cursor-pointer justify-start gap-3 p-0">
                            <input type="checkbox" name="remember" class="checkbox checkbox-error checkbox-sm rounded-lg" />
                    ...
                    <div class="form-control pt-2">
                        <button type="submit" id="submitBtn" class="btn bg-rose-600 hover:bg-rose-700 border-none rounded-2xl text-white font-semibold text-sm shadow-lg shadow-rose-600/30 hover:shadow-rose-600/50 hover:scale-[1.02] active:scale-[0.98] transition-all h-12">
                            Masuk ke Dashboard
                        </button>
                    </div>
                </form>
